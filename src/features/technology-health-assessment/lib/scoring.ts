...
export function scoreCategory(category: AssessmentCategory, answers: Record<string, AssessmentAnswer>, findingsCount = 0): CategoryScore {
  const questionScores = assessmentQuestions
  ...
  const weightedTotal = questionScores.reduce((total, score) => {
    const question = assessmentQuestions.find((item) => item.id === score.questionId);
  ...
  return {
    ...
    answered: assessmentQuestions.filter((question) => question.categoryId === category.id && answers[question.id]).length,
    applicable: questionScores.length,
    findings: findingsCount,
  };
}

export function calculateScores(answers: Record<string, AssessmentAnswer>, findingsByCategory: Record<string, number> = {}): AssessmentScores {
  const questionScores = assessmentQuestions.map((question) => scoreQuestion(question, answers[question.id]));
  const categoryScores = assessmentCategories.map((category) => scoreCategory(category, answers, findingsByCategory[category.id] ?? 0));
  const weightedOverall = categoryScores.reduce((total, score) => {
    const category = assessmentCategories.find((item) => item.id === score.categoryId);
    return total + score.score * (category?.weight ?? 0);
  }, 0);
  const totalWeight = assessmentCategories.reduce((total, category) => total + category.weight, 0);
