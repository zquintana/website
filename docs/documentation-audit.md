...
| Scoring | Partially complete | `lib/scoring-core.ts`, `lib/scoring.ts`, browser import, tests | Shared question/category/overall scoring now exists; report presentation and browser integration remain lightly typed | Yes, with integration risk | Added scoring core and runtime read-model test |
| Findings and risk | Partially complete | `methodology/findings.ts`, `methodology/legacy/findings.ts`, `lib/findings.ts` | Modular and compatibility conditions now execute; legacy content and report presentation remain | Partial | Added shared evaluator and legacy compatibility adapter |
