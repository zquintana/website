...
Evidence levels currently map to confidence as follows: self-reported/unverified 0.40, documentation reviewed 0.65, manually verified 0.80, automatically verified 1.00 (the last is hidden in the MVP UI). The target executive-facing vocabulary is Reported, Observed, and Verified; the implementation identifiers should be mapped to that vocabulary before production reporting. The answer stores only `evidenceLevel` and optional notes; dates, source identity, artifacts, and reviewer identity are not modeled.
