...
The source retains `cataloged` or `implemented` for compatibility and now also carries executable readiness metadata. Documentation uses the richer review vocabulary: Proposed, Structured, Draft questions, Pilot, Active, Deprecated. A catalog entry without a module is `Structured` at most. The three implemented modules—`recovery-capability`, `identity-lifecycle`, and `privileged-access`—are Pilot/Demonstration, not Active production methodology. Readiness status, ownership fields, review date, and notes are stored on each catalog entry; summary and activation checks live in `methodology/readiness.ts`.
