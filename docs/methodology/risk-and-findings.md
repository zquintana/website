...
The authored model supports finding conditions with `all` or `any`, operators `equals`, `in`, `unknown`, `not-applicable`, `lte`, and `gte`, severities, business impact, and recommendation IDs. `methodology/findings.ts` now evaluates these authored definitions, including treating a missing answer as unknown for visibility conditions. The current legacy report generator still uses question-level recommendation and `criticalFindingRule` values after flattening, so wiring the new evaluator into report generation remains a follow-up step.
