...
| Applicability is explicitly evaluated | [Assessment runtime](../architecture/assessment-runtime.md) | `lib/applicability.ts`, questionnaire and scoring paths | profile/answer-rule unit tests and manifest schema | Implemented; no authored active rules yet |
| Capability scores aggregate through primary category only | [Scoring model](../methodology/scoring-model.md) | `methodology/capability-results.ts`, browser report | capability result and primary aggregation tests | Implemented |
| Foundational treatment is explicit per version | [Methodology overview](../methodology/methodology-overview.md) | `AssessmentVersion.foundationalTreatment` | active-version assertion and schema validation | Implemented as metadata-only |
