...
| Capability | id, name, definition, outcome, domain, primary category, scoring model, applicability, owned IDs, readiness metadata | Four implemented; catalog has 142; readiness is executable |
| Question | id, capability, prompt, response type, applicability, options, importance, severity, evidence requirements | 23 read-model questions; pilot module questions include evidence guidance; legacy shape at UI boundary |
