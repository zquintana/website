...
Current sequence differs: Astro serializes the active read model; browser code loads exact `mvp-1` localStorage state, renders category pages, writes answers after each category, calculates directly from flat questions, and builds HTML results. The modular finding evaluator now exists and is covered by methodology tests, but the browser report still uses the legacy finding path. Applicability is metadata-only. Recalculation occurs when results render, not as a separately persisted result.
